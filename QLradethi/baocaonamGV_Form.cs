...
namespace QLradethi
{
    public partial class baocaonamGV_Form : Form
    {
        string magiangvien;
        ...
        public baocaonamGV_Form()
        {
            InitializeComponent();
            sqlCon = new SqlConnection(strCon);
        }
        public baocaonamGV_Form(string magiangvien)
        {
            InitializeComponent();
            sqlCon = new SqlConnection(strCon);
        ...
        private void baocaonamGV_Form_FormClosed(object sender, FormClosedEventArgs e)
        {
            Thoat(this, new EventArgs());
        }
        void LoadData_baocaonamGV_Form()
        {
        ...
        void Load_danhsachmonhoc()
        {
            ...
            if (Convert.ToDecimal(tsbc_label.Text) != 0)
            {
                for (int i = 0; i < danhsachmonhoc.Count; i++)
                {
                    ...
                    cmd = sqlCon.CreateCommand();
                    cmd.CommandText = "SELECT COUNT (*) FROM CT_LOP JOIN LOP ON CT_LOP.MALOP=LOP.MALOP WHERE MAMONHOC='" + danhsachmonhoc[i].mamonhoc + "' AND NAMHOC=" + nam_nud.Value + " AND MAGIANGVIENCHAMTHI='" + magiangvien + "'";
                    cmd.Connection = sqlCon;
            ...
            if (Convert.ToDecimal(tsdt_label.Text) != 0)
            {
                for (int i = 0; i < danhsachmonhoc.Count; i++)
                {
                    ...
                    cmd = sqlCon.CreateCommand();
                    cmd.CommandText = "SELECT COUNT (*) FROM DETHI WHERE MAMONHOC='" + danhsachmonhoc[i].mamonhoc + "' AND NAMHOC=" + nam_nud.Value + " AND MAGIANGVIEN='" + magiangvien + "'";
                    cmd.Connection = sqlCon;
                    Sdr = cmd.ExecuteReader();
                    while (Sdr.Read())
                    {
        ...
        void Load_tongso()
        {
            ...
            cmd = sqlCon.CreateCommand();
            cmd.CommandText = "SELECT count(*) FROM CT_LOP JOIN LOP ON CT_LOP.MALOP=LOP.MALOP WHERE NAMHOC=" + nam_nud.Value + " AND MAGIANGVIENCHAMTHI='" + magiangvien + "'";
            cmd.Connection = sqlCon;
            SqlDataReader Sdr = cmd.ExecuteReader();
            while (Sdr.Read())
            {
            ...
            cmd = sqlCon.CreateCommand();
            cmd.CommandText = "SELECT count(*) FROM DETHI WHERE NAMHOC=" + nam_nud.Value + " AND MAGIANGVIEN='" + magiangvien + "'";
            cmd.Connection = sqlCon;
            Sdr = cmd.ExecuteReader();
            while (Sdr.Read())
            {
        ...
        {
            LoadData_baocaonamGV_Form();
        }

        private void xem_btn_Click_1(object sender, EventArgs e)
        {
            nam_label.Text = nam_nud.Value.ToString();
            Load_tongso();
            Load_danhsachmonhoc();
        ...
        private void quaylai_btn_Click_1(object sender, EventArgs e)
        {
            this.Close();
        }

        private void baocaonamGV_Form_Load(object sender, EventArgs e)
        {
            LoadData_baocaonamGV_Form();
        }
    }
